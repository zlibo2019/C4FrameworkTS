...
import { getModulesEx } from 'c4utils/src/FSPromise';
...
export async function C4InitFlow(helpersName : string[]) {
    ...
    if (!TypeUtils.isArray(HelpersName)
        || (HelpersName.length > 0 && !TypeUtils.isString(HelpersName[0]))) {
        console.log("C4Helper get a bad format config.");
        process.exit(-1);
    }

    if (TypeUtils.isEmptyArray(HelpersName)) {
