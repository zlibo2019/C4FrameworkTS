...
import { Swagger2Object, Swagger2ParameterObject, Swagger2OperationObject, Swagger2SecurifyType, SwaggerRef, Swagger2ParameterObjectExtInfo, Swagger2ParameterNotBodyObject, Swagger2SchemaObject, Swagger2ResponseObject } from './C4SwaggerDefine';
import { Swagger2RenderData, Swagger2RenderMethodData, Swagger2RenderParameter } from './C4SwaggerRenderDefine';


function normalizeName(id : string) {
    return id.replace(/\.|\-|\{|\}|\s/g, '_');
};

function getPathToMethodName(opts : any, m : string, path : string){
    if(path === '/' || path === '') {
        return m;
    }
...
function getSchemaType(schema : Swagger2SchemaObject, swagger : Swagger2Object) : string{
    if (schema.type)
        return schema.type;
    if (schema.$ref) {
        let segments = schema.$ref.split('/');
        let tempRef : Swagger2SchemaObject = {};
        if (segments.length === 3) {
            if (segments[1] === "parameters") {
                if (swagger.parameters)
                    tempRef = <Swagger2SchemaObject>swagger.parameters[segments[2]]
...
export function getViewForSwagger2(opts : {
    ...
    let globalDownloadFile : boolean = false;
    _.forEach(globalResponse, function(rep, name : string) {
        if (name === "200") {
            let CurType = "";
            if (rep.schema) {
                if (rep.schema.$ref)
    ...
    _.forEach (swagger.paths, function (api, path : string){
        let globalParams : (Swagger2ParameterObject | SwaggerRef)[] = [];
        /**
         * @param {Object} op - meta data for the request
         * @param {string} m - HTTP method name - eg: 'get', 'post', 'put', 'delete'
         */
        // 处理所有Method的公共参数
        _.forEach (api, function (op, m : string){
            if (m.toLowerCase() === 'parameters') {
                globalParams = <(Swagger2ParameterObject | SwaggerRef)[]>op;
            }
        });
        _.forEach (api, function (opt, m){
            ...
            let op = <Swagger2OperationObject>opt;
            let secureTypes : Swagger2SecurifyType[] = [];
            ...
            let method : Swagger2RenderMethodData = {
                path: path,
                className: opts.className,
            ...
            _.forEach(responses, function(rep, name : string) {
                if (name === "200") {
                    let CurType = "";
                    if(rep) {
                        if ((<any>rep).schema) {
                            if ((<any>rep).schema.$ref)
                                CurType = getSchemaType((<any>rep).schema, swagger);
                            else if ((<any>rep).schema.type) {
                                CurType = (<any>rep).schema.type;
                            }
                        } else if ((<any>rep).$ref) {
                            CurType = getSchemaType((<any>rep), swagger);
                        }
                        if (CurType === 'file') {
                            isDownloadFile = true;
                        }
            ...
            if (globalDownloadFile) isDownloadFile = true;

            let params : (Swagger2ParameterObject | SwaggerRef)[] = [];
            ...
            _.forEach(params, function (parameter) {
                ...
                if ((<Swagger2ParameterObject>parameter)['x-exclude-from-bindings']
                    && (<Swagger2ParameterObject>parameter)['x-exclude-from-bindings'] === true) {
                    return;
                }

                // Ignore headers which are injected by proxies & app servers
                // eg: https://cloud.google.com/appengine/docs/go/requests#Go_Request_headers
                if ((<Swagger2ParameterObject>parameter)['x-proxy-header'] && !data.isNode) {
                    return;
                }
                if (_.isString((<SwaggerRef>parameter).$ref)) {
                    let segments = (<SwaggerRef>parameter).$ref.split('/');
                    if (swagger.parameters)
                        parameter = swagger.parameters[segments.length === 1 ? segments[0] : segments[2] ];
                }
                let parameterExtInfo : Swagger2ParameterObjectExtInfo = {
                    camelCaseName       : "",
                    isSingleton         : false,
                ...
                parameterExtInfo.camelCaseName = _.camelCase((<Swagger2ParameterObject>parameter).name);
                if((<Swagger2ParameterNotBodyObject>parameter).enum) {
                    if((<any []>(<Swagger2ParameterNotBodyObject>parameter).enum).length === 1) {
                        parameterExtInfo.isSingleton    = true;
                        parameterExtInfo.singleton      = (<any []>(<Swagger2ParameterNotBodyObject>parameter).enum)[0];
                    }
                }
                if((<Swagger2ParameterObject>parameter).in === 'body'){
                    parameterExtInfo.isBodyParameter = true;
                } else if((<Swagger2ParameterObject>parameter).in === 'path'){
                    parameterExtInfo.isPathParameter = true;
                } else if((<Swagger2ParameterObject>parameter).in === 'query'){
                    if((<Swagger2ParameterObject>parameter)['x-name-pattern']){
                        parameterExtInfo.isPatternType = true;
                        parameterExtInfo.pattern = (<Swagger2ParameterObject>parameter)['x-name-pattern'];
                    }
                    parameterExtInfo.isQueryParameter = true;
                } else if((<Swagger2ParameterObject>parameter).in === 'header'){
                    parameterExtInfo.isHeaderParameter = true;
                } else if((<Swagger2ParameterObject>parameter).in === 'formData'){
                ...
                parameterExtInfo.cardinality = (<Swagger2ParameterObject>parameter).required ? '' : '?';
                let CurRenderParams : Swagger2RenderParameter = <Swagger2RenderParameter>_.assign(parameter, parameterExtInfo);
